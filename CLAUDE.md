...
**Sprint 7 (P0) 100% concluído (2026-08-27, mesmo dia):**

- **Termos de Uso + Política de Privacidade** (`app/src/legal.js`, rotas `/termos` e `/privacidade`, públicas): adaptadas do documento real já em produção do VYRON (mesmo padrão de negócio individual, mesma estrutura LGPD). Tabela de prestadores de dados nomeia Supabase/Anthropic/Asaas de forma factual, sem nunca escrever a palavra "IA" — mesmo padrão que o VYRON já usa (nomear o prestador e a função, não a tecnologia). Linkadas no rodapé da landing e como aceite no formulário de cadastro.
- **Analytics de produto**: reaproveita a tabela `anuncia_usage_events` já existente (sem infra nova) — 3 eventos novos (`cadastro`, `primeiro_imovel`, `exportacao`, `assinatura`) somados aos que já existiam (`geracao`, `regeneracao`). `cadastro` é dado pelo frontend logo após `supabase.auth.signUp()` (o signup em si não passa pelo backend); `primeiro_imovel` dispara só na 1ª propriedade do usuário (`count === 1` depois do insert); `exportacao` no download; `assinatura` no endpoint admin de troca de plano (e vai capturar o webhook real do Asaas no P1 também).
- **Página de exemplos** (`app/src/examples.js`, rota `/exemplos`, pública): 2 imóveis fictícios completos (apartamento + casa), cada um com 4 materiais no mesmo tratamento "manuscrito" do editor real, badge "exemplo ilustrativo — imóveis, preços e corretores fictícios" bem visível, CTA final pro cadastro. Linkada a partir da seção de demonstração da landing.

Testado de ponta a ponta (curl + navegador real): as 3 páginas novas renderizam e navegam certo (voltar, links cruzados Termos↔Privacidade, CTA de exemplos→cadastro), os 5 tipos de evento de analytics confirmados na ordem certa com metadata correta num fluxo real (cadastro→imóvel→geração→exportação→troca de plano). Banco de teste limpo depois.
